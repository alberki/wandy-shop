...
class Product extends MY_Controller
{
    ...
    public function index($page = null)
    {
        ...
        $data['content'] = $this->product->select(
            [
                ...
                'category.title AS category_title', 'product.stock AS product_stock'
            ]
        )->join('category')
            ->paginate($page)
            ->get();
    ...
    public function search($page = null)
    {
        ...
        )->join('category')->like('product.title', $keyword)->orLike('description', $keyword)
            ->paginate($page)
            ->get();
    ...
    public function create()
    {
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] != '') {
            $imageName = url_title($input->title, '_', true) . '_' . date('YmdHis');
            $upload = $this->product->upload_image('image', $imageName);
    ...
    public function edit($id)
    {
        ...
        if ($this->product->where('id', $id)->update($data['input'])) {
            $this->session->set_flashdata('success', 'Data Produk berhasil di Ubah');
        } else {
            $this->session->set_flashdata('error', 'Data Produk gagal di Ubah');
        }
        redirect(base_url('product'));
    }
