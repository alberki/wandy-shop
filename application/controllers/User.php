...
class User extends MY_Controller
{
    ...
    public function search($page = null)
    {
        ...
        $data['content'] = $this->user->like('name', $keyword)->orLike('email', $keyword)
            ->paginate($page)
            ->get();
    ...
    public function create()
    {
        if (!$_POST) {
            ...
            // $this->form_validation->set_rules('password_confirmation', 'Konfirmasi Password', 'trim|required|matches[password]');
            $input->password = hashEncrypt($input->password);
        }

        if (!empty($_FILES) && $_FILES['image']['name'] != '') {
            $imageName = url_title($input->name, '_', true) . '_' . date('YmdHis');
            $upload = $this->user->upload_image('image', $imageName);
    ...
    public function edit($id)
    {
        ...
        if (!empty($_FILES) && $_FILES['image']['name'] != '') {
            $imageName = url_title($data['input']->title, '_', true) . '_' . date('YmdHis');
            $upload = $this->user->upload_image('image', $imageName);
        ...
        if ($this->user->where('id', $id)->update($data['input'])) {
            ...
            $this->session->set_flashdata('error', 'Data User gagal di Ubah');
        }
        redirect(base_url('user'));
    }
    ...
    public function unique_email()
    {
        ...
        if ($user) {
            ...
            $this->form_validation->set_message('unique_email', '%s sudah di gunakan');
            return false;
        } else {
            return true;
        }
